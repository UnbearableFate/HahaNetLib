...
public:
	...
	Channel(int fd = -1) : fd(fd), readCallback(nullptr), writeCallback(nullptr), errorCallback(nullptr),event(0),revent(0) {}
	...
	void handleIO();
	void enableEvent(int e) { event |= e; this->update(); }
	void disableEvent(int e) { event &= ~e; this->update(); }
